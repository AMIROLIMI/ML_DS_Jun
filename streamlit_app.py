...
def main():
    ...
    if url:
        ...
        (странно что data.info() не отображается и показывает None)
        """)
        st.text(data.info())
        ...
        """)
        st.markdown("""
        ### Cначало удалю реально категориальные признаки, чтобы было удобно:
        поля которые удалятся при нажатии на кнопку: ["A1", "A4", "A5", "A6", "A7", "A9", "A10", "A12", "A13"]

        Кроме того после нажатии на кнопку удалятся в данных неправильные значения как "?". Эти значения будут заменены средними значениями.
        """)
