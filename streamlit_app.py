...
def main():
    ...
    if url:
        ...
        evaluate_models(knc, log_reg, dtc, X_train, X_test, y_train, y_test)
        st.write("Видно что у маделей KNN и Decission Tree переобучение так как у них на трейне высокий показатель а на тесте низкий. А у модели Logistic Regression такого нет. по этому будем считать что самый хорошый модель это - Logistic Regression (это говорится про то модели которые у них гиперпараметры стоят по умолчанию. так как есть возможность менять гиперпараметры и прочее, то при их изменения рассуждения про моделей будут вообще другим)")
